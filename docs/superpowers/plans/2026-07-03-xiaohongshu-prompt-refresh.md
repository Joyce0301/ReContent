...
**Architecture:** Keep the existing unified prompt builder and only rewrite the Xiaohongshu platform instruction text plus a few shared guidance lines. Extend prompt-builder tests to lock in the new length, structure, and tone constraints while preserving conservative-mode JSON safety.
...
```ts
- 小红书笔记：1 篇中文笔记，包含一个有吸引力的标题（不超过 20 字）和正文（约 700-1200 字）。标题优先体现人群、场景、痛点或收获感；正文采用短段落表达，从具体场景、问题、感受或观察切入，像真人分享经验而不是复述原文，至少 3 个展开段，每段尽量提供解释、例子、方法、对比或适用场景，保持真诚自然和弱营销感，可使用少量 emoji，并在结尾带上 3-5 个强相关标签。
```

- [ ] **Step 2: Strengthen the shared guidance so the model prefers explanation over summary**

Update the shared guidance block by replacing:
...
```ts
- 保留原文的核心观点和数据，但用更适合社交平台的方式表达，优先写出具体场景、展开解释和可感知的细节，不要只做压缩式摘要。
```
