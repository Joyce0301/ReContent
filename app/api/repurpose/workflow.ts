...
export type RunRepurposeWorkflowResult = {
  results: ParsedRepurposeResponse["results"];
};

export class UrlExtractionWorkflowError extends Error {
  diagnostics: ExtractionDiagnostics | null;

  constructor(diagnostics: ExtractionDiagnostics | null) {
    super("URL extraction failed");
    this.name = "UrlExtractionWorkflowError";
...
export async function runRepurposeWorkflow(
  input: RunRepurposeWorkflowInput
): Promise<RunRepurposeWorkflowResult> {
  let extractionDiagnostics: ExtractionDiagnostics | null = null;
  const sourceContent =
    input.mode === "text"
      ? input.text?.trim() ?? ""
      : await extractUrlContent(input.url ?? "", diagnostics => {
  ...
  if (!sourceContent) {
    ...
    throw new UrlExtractionWorkflowError(extractionDiagnostics);
  }

  const provider = getAiProvider();
  const results =
    provider === "mock"
      ? generateMockResults(sourceContent, input.platform, input.tone)
...
function generateMockResults(source: string, platform: PlatformKey, tone: ToneKey) {
